...
This script will remove the beanstalk directory from the appliance.
...
echo "This script will remove the beanstalk directory from the appliance" $appliance"."
echo ""
...
if [[ $remove =~ ^([yY][eE][sS]|[yY])$ ]]; then
    cp rm-beanstalk.yml.tmpl rm-beanstalk-$appliance.yml
    sed -i '' "s~{{APPLIANCE}}~$appliance~; s~{{NAMESPACE}}~$namespace~" rm-beanstalk-$appliance.yml
    kubectl create -f rm-beanstalk-$appliance.yml
    sleep 7
    rm rm-beanstalk-$appliance.yml
