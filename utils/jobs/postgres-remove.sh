...
This script will remove the postgres directory from the appliance.
...
echo "This script will remove the postgres directory from the appliance" $appliance"."
echo ""
...
if [[ $remove =~ ^([yY][eE][sS]|[yY])$ ]]; then
    cp postgres-remove.yml.tmpl postgres-remove-$appliance.yml
    sed -i '' "s~{{APPLIANCE}}~$appliance~; s~{{NAMESPACE}}~$namespace~" postgres-remove-$appliance.yml
    kubectl create -f postgres-remove-$appliance.yml
    sleep 7
    rm postgres-remove-$appliance.yml
