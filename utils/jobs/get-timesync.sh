...
This script will report the status of the timesyncd service from the appliance.
...
echo "This script will report the status of the timesyncd service from the appliance" $appliance" to the newsroom."
echo ""
...
if [[ $get =~ ^([yY][eE][sS]|[yY])$ ]]; then
    cp get-timesync.yml.tmpl get-timesync-$appliance.yml
    sed -i '' "s~{{APPLIANCE}}~$appliance~; s~{{NAMESPACE}}~$namespace~" get-timesync-$appliance.yml
    kubectl create -f get-timesync-$appliance.yml
    sleep 10
    rm get-timesync-$appliance.yml
    kubectl --namespace=$namespace get jobs
    kubectl --namespace=$namespace delete jobs get-timesync-$appliance
else
    exit 1
fi
