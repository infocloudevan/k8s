...
This script will backup the postgres directory from the appliance to the /home/core directory.
...
echo "This script will backup the postgres directory from the appliance "$appliance" to the /home/core directory."
echo ""
...
if [[ $remove =~ ^([yY][eE][sS]|[yY])$ ]]; then
    cp postgres-backup.yml.tmpl postgres-backup-$appliance.yml
    sed -i '' "s~{{APPLIANCE}}~$appliance~; s~{{NAMESPACE}}~$namespace~" postgres-backup-$appliance.yml
    kubectl create -f postgres-backup-$appliance.yml
    sleep 7
    rm postgres-backup-$appliance.yml
