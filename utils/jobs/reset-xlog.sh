...
This script will reset the postgres xlog on the appliance.
...
echo "This script will reset the postgres xlog on the appliance" $appliance"."
echo ""
...
if [[ $remove =~ ^([yY][eE][sS]|[yY])$ ]]; then
    echo "Scaling the main deployment to zero (0)"
    kubectl --namespace "$namespace" scale deployments "$appliance" --replicas=0
    kubectl --namespace "$namespace" get deployments "$appliance"
    sleep 2
    cp reset-xlog.yml.tmpl reset-xlog-$appliance.yml
    sed -i '' "s~{{APPLIANCE}}~$appliance~; s~{{NAMESPACE}}~$namespace~" reset-xlog-$appliance.yml
    kubectl create -f reset-xlog-$appliance.yml
    sleep 7
    rm reset-xlog-$appliance.yml
